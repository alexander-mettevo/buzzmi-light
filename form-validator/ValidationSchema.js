...
import * as validationFunctions from './validators/builtInValidators.js';
import {defaultErrorMessages} from "./validators/defaultErrorMessages.js";
/**
...
class ValidationSchema {
   ...
   */
  constructor(rules = {}, defaultMessages = defaultErrorMessages) {
    this.rules = rules;
    ...
    for (const { rule, message, value: ruleValue } of fieldRules) {
      ...
      try {
        if (validationFn) {
          ...
          if (!isValid) {
            // Use the default message if the message is not provided
            const defaultMessage = this.defaultMessages[rule];
            errors.push(message || (typeof defaultMessage === 'function' ? defaultMessage(ruleValue) : defaultMessage));
          }
        } else {
  ...
  getFieldMessages(fieldName) {
    const fieldRules = this.getFieldRules(fieldName);
    return fieldRules.map(({ rule, message, value: ruleValue }) => {
      const defaultMessage = this.defaultMessages[rule];
      return (
        message ||
        (typeof defaultMessage === 'function'
          ? defaultMessage(ruleValue)
          : defaultMessage)
      );
    });
  }
